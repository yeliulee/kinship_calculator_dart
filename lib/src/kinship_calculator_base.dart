import 'kinship_option.dart';

/// [KinshipCalculator]
/// Kinship Calculator 亲戚计算器
class KinshipCalculator {
  static const String _KEY_REGEXP = 'exp';
  static const String _KEY_STRING = 'str';

  /// 称呼简写过滤字典
  static final List<Map<String, dynamic>> _filter = [
    /* 表亲 */
    {
      //表亲的关系
      'exp': r'^(.+)&o([^#]+)&l/g',
...
/* 兄弟姐妹 */
    ...
    },
    {
      ...
      'str': (Match m) {
        return '${m[1]}${m[2]},xb${m[4]}#${m[1]}${m[2]}${m[4]}';
      }
    },
    {
      ...
      'str': (Match m) {
        return '${m[1]}${m[2]},xs${m[4]}#${m[1]}${m[2]}${m[4]}';
      }
    },
    {
      //如果自己是男性,兄弟姐妹的姐妹就是自己的姐妹
      'exp': r'(,[fh1])(,[olx][sb])+,[olx]s/g',
      'str': (Match m) {
        return '${m[1]},xs';
      }
    },
...
/* 父母的子女 */
    ...
    },
    {
      ...
      'str': (Match m) {
        return '${m[1]}${m[2]},xb${m[3]}#${m[1]}${m[2]}${m[3]}';
      }
    },
    {
      ...
      'str': (Match m) {
        return '${m[1]}${m[2]},xs${m[3]}#${m[1]}${m[2]}${m[3]}';
      }
    },
    {
      //如果自己是女性,父母的儿子是自己兄弟
...
/* 夫妻 */
  ...
  static final Map<String, List<String>> _data = {
    '': ['自己', '我'],
    //并称
  ...
  /// 数组去重
  static List<String> _unique(List<String> array) {
    var result = <String>[];
    ...
    for (var i = 0; i < array.length; i++) {
      ...
      /// 对特殊语法标识相互包含的行为去重
      var temp = item
          .replaceFirst(RegExp('[ol](?=s|b)'), 'x')
          .replaceFirst(RegExp('&[ol]'), '');
      if ((hash[item] == null || !hash[item]) &&
          (hash[temp] == null || !hash[temp])) {
        result.add(item);
        hash[item] = true;
      }
    }
  ...
  /// 分词解析
  static List<String> _getSelectors(String str) {
    str = str.replaceAll(RegExp('[二|三|四|五|六|七|八|九|十]{1,2}'), 'x');
    var words = str
        .replaceFirst('我', '')
        .replaceFirst('家的', '的')
        .replaceFirst('家', '的')
    ...
    while (words.isNotEmpty) {
      ...
      _data.forEach((key, value) {
        ...
        if (value.contains(name)) {
          /// 对‘我’的优化
          if (key.isNotEmpty || words.isEmpty) {
            arr.add(key);
          }
          has = true;
        }
  ...
  /// 简化选择器
  static List<String> _selector2id(String selector, int sex) {
    var result = <String>[];
    var hash = <String, bool>{};
    ...
    /// 同志关系去除
    if (RegExp(',[w0],w|,[h1],h').allMatches(selector).isNotEmpty) {
      return <String>[];
    }

    dynamic getId(String selector) {
      var s = '';
      if (hash[selector] == null || !hash[selector]) {
        hash[selector] = true;
        var status = true;
        do {
          s = selector;
          for (var i = 0; i < _filter.length; i++) {
            var filterItem = _filter[i];
            if (filterItem[_KEY_REGEXP].endsWith('/g')) {
              var pattern =
                  RegExp(filterItem[_KEY_REGEXP].replaceAll('/g', ''));
              selector =
                  selector.replaceAllMapped(pattern, filterItem[_KEY_STRING]);
            } else {
              var pattern = RegExp(filterItem[_KEY_REGEXP]);
              selector =
                  selector.replaceFirstMapped(pattern, filterItem[_KEY_STRING]);
            }
        ...
        if (status) {
          ...
          /// 去前面逗号和性别信息
          if (selector
              .replaceFirstMapped(RegExp(',[01]'), (match) => '')
              .isNotEmpty) {
            selector = selector
                .replaceFirstMapped(RegExp(',[01]'), (match) => '')
                .substring(1);
  ...
  /// 获取数据
  static List<String> _getDataById(String id) {
    var items = <String>[];
    List<String> getData(String str) {
      var res = <String>[];
      _data.forEach((key, value) {
        if (key.replaceAll(RegExp('&[olx]'), '') == str) {
          res.add(value[0]);
        }
      });
      return res;
    ...
    if (_data.containsKey(id)) {
      items.add(_data[id][0]);
    } else {
      items = getData(id);

      /// 忽略年龄条件查找
      if (items.isEmpty) {
        id = id.replaceAll(RegExp('&[ol]'), '');
        items = getData(id);
      }

      /// 忽略年龄条件查找
      if (items.isEmpty) {
        id = id.replaceAll(RegExp('[ol]'), 'x');
        items = getData(id);
      }

      /// 缩小访问查找
      if (items.isEmpty) {
        var l = id.replaceAll(RegExp('x'), 'l');
        items = getData(l);
        var o = id.replaceAll(RegExp('x'), 'o');
        items.addAll([...getData(o)]);
      }
    }
    return items;
  }
  ...
  /// 逆转 ID
  static String _reverseId(String id, int sex) {
    var hash = <String, List<String>>{
      'f': ['d', 's'],
      'm': ['d', 's'],
      'h': ['w', ''],
      'w': ['', 'h'],
    ...
    if (id != null && id.isNotEmpty) {
      ...
      /// 逆转运算自身性别必须确定
      sex = (sex != null && sex != 0) ? 1 : 0;
      var sid = (',' + sex.toString() + ',' + id)
          .replaceAll(RegExp(',[fhs]|,[olx]b'), ',1')
          .replaceAll(RegExp(',[mwd]|,[olx]s'), ',0');
      sid = sid.substring(0, sid.lastIndexOf(','));
      var idList = id.split(',').reversed.toList();
      var sidList = <int>[];
      sid.split(',').reversed.toList().forEach((e) {
        if (e.isNotEmpty) {
          sidList.add(int.parse(e));
        }
      });
      var arr = <String>[];
      for (var i = 0; i < idList.length; i++) {
        arr.add(hash[idList[i]][sidList[i]]);
      }
      return arr.join(',') + age;
    }
    return '';
  ...
  ///获取关系链条
  static String _getChainById(String id) {
    var arr = id.split(',');
    var items = <String>[];
    for (var i = 0; i < arr.length; i++) {
      var key = arr[i].replaceFirst(RegExp('&[ol]'), '');
      items.add(_data[key][0]);
    }
    return items.join('的');
  }

  /// 计算
  static List<String> calculate(KinshipOption option) {
    var text = option.text;
    var sex = option.sex;
    var type = option.type;
    var reverse = option.reverse;

    /// 'type' 为 'chain' 时, 'reverse' 无效

    var selectors = _getSelectors(text);
    var result = <String>[];

    ///匹配结果
    selectors.forEach((selector) {
      ///遍历所有可能性
      var ids = _selector2id(selector, sex);
      ids.forEach((id) {
        if (type == 'chain') {
          var item = _getChainById(id);
          if (item.isNotEmpty) {
            result.add(item);
          }
        } else {
          if (reverse) {
            id = _reverseId(id, sex);
          }
          ...
          if (items.isNotEmpty) {
            ...
            ///找不到关系，随爱人叫
            items = _getDataById(id.substring(2));
            if (items.isNotEmpty) {
              result.addAll(items);
            }
          }
